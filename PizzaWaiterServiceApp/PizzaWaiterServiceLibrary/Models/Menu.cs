...
namespace Models
{
    ...
    public class Menu : SqlModel
    {
        ...
        public void BuildObject(DataRow row)
        {
            ...
            this.Title = SqlFormat.ToString(row, "Title");
        }
        ...
        /* Adds menu to db */
        public Response<Menu> Create()
        {
            this.Connect();
            return this.menuDB.Create(this);
        }

        /* Updates menu in database */
        public Response<Menu> Update()
        {
            this.Connect();
            return this.menuDB.Update(this);
        }

        /*Deletes menu from database
         * add here manual cascades, if are needed
         */
        public Response<Menu> Delete()
        {
            this.Connect();
            return this.menuDB.Delete(this);
        }

    }

    /* Communicates object to database */
    public class MenuDB : SqlHandler<Menu>
    {

        /* translate data from c# to sql */
        private SqlData SetData(Menu i)
        {
        ...
        private int Validate(Menu menu, params Input[] inputs)
        {
            ...
            this.Response = new Response<Menu>();
                ...
                foreach (Input input in inputs)
                {
                    switch (input)
                    {
                        ...
                        case Input.TitleIsNull:
                            if (this.ValidateTitleIsNull(menu))
                            {
                                this.Response.AddMessage(ResponseMessage.DataEmpty); // add message
                                err++; // count errors up
                            }
        ...
        private bool ValidateIdIsNull(Menu menu)
        {
            return (menu.ID == null || menu.ID == 0);
        }
        private bool ValidateTitleIsNull(Menu menu)
        {
            return (menu.Title == null || menu.Title == "");
        }
        #endregion
        #endregion

        public Response<Menu> Update(Menu menu)
        {
            /* run validation
             * check that id and name are filled up)
             * */
            int err = this.Validate(menu, Input.IdIsNull, Input.TitleIsNull);
            // if both fields are filled up, try to update the menu 
            if (err < 1)
            {
                SqlData data = this.SetData(menu); // translate c# to Sql
                this.Update(data, menu.ID); // run transaction
            ...
            /* finish by adding the final menu to response, 
             * (validation may correct some data without giving a visible to user error)
             * so we can use both the menu and the messages later
             * */
            this.Response.Item = menu;
        ...
        public Response<Menu> Delete(Menu menu)
        {
            ...
            int err = this.Validate(menu, Input.IdIsNull);
            if (err < 1)
            {
                int rowsAffected = this.Delete(menu.ID); // get rows deleted
        ...
        public Response<Menu> Create(Menu menu)
        {
            //same procedure as update, just dont need id validation
            int err = this.Validate(menu, Input.TitleIsNull);


            if (err < 1)
            {
                SqlData data = this.SetData(menu);
                menu.ID = this.InsertScopeId(data);
            }
